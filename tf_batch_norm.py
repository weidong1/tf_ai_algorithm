...
class NeuralNetWork():
    ...
    def fully_connected(self, layer_in, layer_weights, activation_fn=None):
        ...
        if self.use_batch_norm and activation_fn:
            ...
            # 调用BN接口，
            # 训练时，要设置为True，保证在训练过程中使用的是mini-batch的统计量进行normalization；
            # 在Inference阶段，使用False，也就是使用总体样本的无偏估计。
    ...
    def train(self, sess, learning_rate, training_batches, batches_per_validate_data, save_model=None):
        ...
        # 官方文档指出用BN时moving_mean and moving_variance need to be updated，
        # the update ops are placed in tf.GraphKeys.UPDATE_OPS
        # 要使用tf.control_dependencies保证模型正常训练。
        ...
        # 显示进度条
        for i in tqdm.tqdm(range(training_batches)): # tqdm函数只要传入的是list就行
